...
export interface TerminalContextViewProps {
  ...
  status: HelperStatus; command: string; error?: string; warning?: string;
  explorerLabel: string; canExplore: boolean; canAgent: boolean; canPopout: boolean; canIframe: boolean;
  children: ReactNode;
  onClose(): void; onCopyRef(): void; onCopyPath(): void; onExplore(): void;
  onWatch(): void; onTodo(): void; onPort(entry: PortUrlEntry, mode: PortMode): void;
  onModify(command: string): Promise<void>; onReset(): Promise<void>; onPromote(): Promise<void>;
...
export function TerminalContextView(p: TerminalContextViewProps) {
  ...
  const run = async (action: () => Promise<void>) => { setBusy(true); setError(''); try { await action(); setDetail(null); } catch (e) { setError(String(e instanceof Error ? e.message : e)); } finally { setBusy(false); } };
  ...
  return <section aria-label="Terminal context" data-terminal-context tabIndex={-1}
    ...
    onKeyDown={event => {
      ...
      if (detail && event.key === 'Tab') {
        const controls = Array.from(detailRoot.current?.querySelectorAll<HTMLElement>('button:not(:disabled),input,select') ?? []);
        ...
        <div className="flex min-h-7 flex-wrap items-center gap-2">
          {p.scan.status === 'scanning' ? <span className="text-muted">Scanning ports…</span> : p.scan.status === 'failed' ? <span className="text-error">Port scan failed · Reopen to try again</span> : !selected ? <span className="text-muted">No listening ports</span> : <>
            ...
            <div className="ml-1 inline-flex shrink-0 items-center gap-1 border-l border-border pl-2">
              <ContextAction label="Open in system browser" onClick={() => p.onPort(selected, 'system')}><ArrowSquareOutIcon size={15} />System browser</ContextAction>
              <ContextAction label={p.canIframe ? 'Open in iframe embed' : 'Iframe unavailable on this host'} disabled={!p.canIframe} onClick={() => p.onPort(selected, 'iframe')}><FrameCornersIcon size={15} />Iframe</ContextAction>
              <ContextAction label={p.canAgent ? 'Open in agent-browser screencast' : 'Agent browser unavailable on this host'} disabled={!p.canAgent} onClick={() => p.onPort(selected, 'ab-screencast')}><AgentRobotIcon size={17} />Agent browser</ContextAction>
              <ContextAction label={p.canPopout ? 'Open in agent-browser popout' : 'Popout unavailable on this host'} disabled={!p.canPopout} onClick={() => p.onPort(selected, 'ab-popout')}><AgentRobotIcon size={17} /><ArrowSquareOutIcon size={13} />Popout</ContextAction>
            </div>
          </>}
        </div>
    ...
    <div className="@container flex min-h-0 flex-1 flex-col border-t border-border">
      <div aria-label="Helper terminal status" className="flex h-9 shrink-0 items-center gap-3 whitespace-nowrap px-3">
        ...
        <div className="ml-auto shrink-0"><ContextAction label="Move this terminal into a new pane" disabled={busy} onClick={() => void run(p.onPromote)}><ArrowLineUpIcon size={15} />Promote</ContextAction></div>
      </div>
      ...
      {(p.warning || p.error || error) && <div role="alert" className="mx-3 mb-2 border-l-4 border-error bg-error/10 px-3 py-2">{p.warning || p.error || error}</div>}
