...
export function Baseboard({ items, activeId, onReattach, notice }: BaseboardProps) {
  ...
  return (
    ...
    >
      ...
      <button ref={arrowMeasureEl} className="absolute -left-[9999px] flex h-5 shrink-0 items-center gap-1 rounded px-1.5 pb-px text-xs font-medium font-mono tracking-[0.06em] text-muted" aria-hidden tabIndex={-1}>
        9 more <CaretRightIcon size={10} weight="bold" />
      </button>

      {items.length === 0 && showHint && (
        <span className="truncate pb-1 text-xs font-mono tracking-[0.06em] text-muted">
          {shortcutHint}
        </span>
      )}
      ...
      {hiddenLeft > 0 && (
        <button
          className="flex h-5 shrink-0 items-center gap-1 rounded px-1.5 pb-px text-xs font-medium font-mono tracking-[0.06em] text-muted transition-colors hover:bg-surface-raised hover:text-foreground"
          onClick={scrollLeft}
        >
      ...
      {hiddenRight > 0 && (
        <button
          className="ml-auto flex h-5 shrink-0 items-center gap-1 rounded px-1.5 pb-px text-xs font-medium font-mono tracking-[0.06em] text-muted transition-colors hover:bg-surface-raised hover:text-foreground"
          onClick={scrollRight}
        >
          {hiddenRight} more
