...
- **Dimensions:** `h-6` (24px), `min-w-[68px]`, `max-w-[220px]`; the title button pads `pl-2.5` (10px) and `gap-2` between its glyph, title, and badges, ending `pr-2.5` alone or `pr-1` when the notepad button follows it. The notepad button then carries the trailing inset itself (`pl-0.5 pr-2`), so the badges keep breathing room from the glyph without the door growing.
- **Type:** `text-sm font-medium font-mono`.
- **Content:** leading browser-display glyph on a browser Surface (`size={12}`, named in the Door's accessible name — `docs/specs/dor-browser.md` → Browser Chrome); truncated title; optional TODO pill (`text-xs font-semibold tracking-[0.08em]`, success-tinted when flourishing); optional bell icon (`size={11}`, `weight="fill"`), `text-alarm-vs-door` when ringing; trailing notepad button (`size={12}`, `weight="fill"`) when the minimized Surface holds notes.
