...
Source of truth: `explainTerminalTitle` in `lib/src/lib/terminal-state.ts`; `cwdDisplay` in `lib/src/lib/terminal-state.ts`; `TerminalContext` in `lib/src/components/wall/TerminalContext.tsx`.
