...
- **FAIL IF** a fragment's first line is not exactly `VERDICT: PASS`, `VERDICT: FAIL`, or `VERDICT: INCONCLUSIVE`, or the reporting step accepts status text other than literal `PASS`/`FAIL`, ignores a dissenting or inconclusive domain, or treats an unreadable fragment as passing (rationale).
